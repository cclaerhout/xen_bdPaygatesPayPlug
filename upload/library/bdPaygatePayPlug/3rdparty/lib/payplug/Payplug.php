...
class Payplug {
	...
	$curl_ssl_vp_state = !XenForo_Application::get('options')->get('bdPaygatePayPlug_disable_curl_ssl_vp');
        ...
        curl_setopt($process, CURLOPT_SSL_VERIFYPEER, $curl_ssl_vp_state); // true to false on local
        curl_setopt($process, CURLOPT_SSL_VERIFYHOST, 2); //was true => set to 2 //http://php.net/manual/en/function.curl-setopt.php
	...
	{
	        curl_close($process);
	        throw new NetworkException("CURL error: no anwser from the url. If you're working on local, you might need to set CURLOPT_SSL_VERIFYPEER to FALSE");
	}
